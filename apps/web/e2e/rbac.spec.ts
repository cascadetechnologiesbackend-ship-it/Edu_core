...
test.describe('Role-Based Access Control (RBAC) & Redirection Workflows', () => {
  ...
  test('admin role redirects to /dashboard and sees admin dashboard with full menus', async ({ adminPage }) => {
    await adminPage.goto('/dashboard');
    await expect(adminPage.locator('h1')).toContainText('Good morning, Admin 👋');

    // Sidebar should contain admin links
    await expect(adminPage.locator('nav a:has-text("HR & Payroll")')).toBeVisible();
  ...
  test('teacher role redirects to /dashboard and sees teacher dashboard, hiding admin-only menus', async ({ teacherPage }) => {
    await teacherPage.goto('/dashboard');
    await expect(teacherPage.locator('h1')).toContainText('Welcome back, Teacher 👋');
    ...
    await expect(teacherPage.locator('nav a:has-text("HR & Payroll")')).toBeHidden();
    await expect(teacherPage.locator('nav a:has-text("Fees")')).toBeHidden();
    await expect(teacherPage.locator('nav a:has-text("Settings")')).toBeHidden();
  });

  test('parent role redirects to /portal and is blocked from admin pages', async ({ parentPage }) => {
