...
test.describe('DPDP Compliance Workflows', () => {
  ...
  test('parent withdrawing consent should block processing', async ({ parentPage }) => {
    await parentPage.goto('/privacy/consent');
    // Check if toggle exists
    const toggle = parentPage.locator('input[type="checkbox"][name="photos_videos"]');
    if (await toggle.isVisible()) {
      ...
      // Confirm success
      await expect(parentPage.locator('text=Preferences updated')).toBeVisible();
