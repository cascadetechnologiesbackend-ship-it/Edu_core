...
test.describe('Academics Module E2E Flow', () => {
  ...
  test('should allow admin to manage subjects, mapping, and timetables', async ({ adminPage }) => {
    ...
    await expect(adminPage.locator('h2')).toContainText('Subject Master Directory');
    ...
    await expect(adminPage.locator('h3:has-text("Add Subject Master Entry")')).toBeHidden();
    ...
    await expect(adminPage.locator('h2')).toContainText('Subject-Teacher Mapping');
    ...
    await adminPage.locator('select').nth(1).selectOption({ label: 'SCI201 - Science Practical' });

    // Submit mapped classroom and subject
    await adminPage.click('button[type="submit"]');
    await expect(adminPage.locator('h3:has-text("Map Subject to Class")')).toBeHidden();

    // 4. Manage Timetable Grid
    await adminPage.click('button:has-text("Timetable Grid")');
    await expect(adminPage.locator('h2')).toContainText('Weekly Timetable Grid');

    // Locate Monday Period 1 cell
    const mondayP1Cell = adminPage.locator('tbody tr').first().locator('td').nth(1);
    ...
    await expect(adminPage.locator('h3:has-text("Schedule Period")')).toBeHidden();
    
    // Grid cell should no longer be "+ Add" (empty placeholder)
    await expect(mondayP1Cell).not.toContainText('+ Add');
  });

  test('should allow teacher to publish assignments', async ({ teacherPage }) => {
    await teacherPage.goto('/academics');
    await expect(teacherPage.locator('h1')).toContainText('Academic Management');

    await teacherPage.click('button:has-text("Assignments & Grading")');
    await expect(teacherPage.locator('h2')).toContainText('Assignments & Home Work');

    const newAssignBtn = teacherPage.locator('button:has-text("New Assignment")');
    ...
    await teacherPage.locator('textarea[required]').fill('Solve all exercises in chapter 3.');
    await teacherPage.click('button[type="submit"]');
    await expect(teacherPage.locator('h3:has-text("Create Assignment")')).toBeHidden();

    // Verify it is created in the sidebar list
    await expect(teacherPage.locator('h3:has-text("Assignments") + div')).toContainText('Math Chapter 3 Homework');
  });
