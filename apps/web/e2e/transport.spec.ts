...
test.describe('Transport Module Flow', () => {
  ...
  test('admin can view vehicles tab and see seeded vehicle', async ({ adminPage }) => {
    ...
    await expect(adminPage.locator('text=Vehicles & Drivers Directory')).toBeVisible({ timeout: 10000 });
  ...
  test('admin can view routes tab and see seeded route with stops', async ({ adminPage }) => {
    await adminPage.goto('/transport');
    ...
    // Verify seeded route
    await expect(adminPage.locator('text=Route A - Hinjewadi')).toBeVisible({ timeout: 10000 });
  ...
  test('admin can add a new vehicle with encrypted driver PII', async ({ adminPage }) => {
    await adminPage.goto('/transport');
    ...
    // Fill the modal form
    const modal = adminPage.locator('form:has(h3:has-text("Register New Vehicle"))');
    await expect(modal).toBeVisible();

    await modal.locator('input[placeholder*="BUS-03"]').fill('BUS-03');
    await modal.locator('input[placeholder*="MH-12-AB-1234"]').fill('MH-14-CD-5678');
    ...
    await expect(adminPage.locator('text=BUS-03')).toBeVisible({ timeout: 10000 });
  });

  test('bus pass tab shows DPDP consent indicator for students', async ({ adminPage }) => {
    await adminPage.goto('/transport');

    // Switch to Bus Passes tab
    await adminPage.click('button:has-text("Bus Passes")');

    await expect(adminPage.locator('text=Student Bus Passes Directory')).toBeVisible({ timeout: 10000 });

    // Click "Map Student / Issue Pass"
    const mapBtn = adminPage.locator('button:has-text("Map Student / Issue Pass")');
    await expect(mapBtn).toBeVisible();
    await mapBtn.click();

    // Modal should open
    const modal = adminPage.locator('form:has(h3:has-text("Map Student & Issue Pass"))');
    await expect(modal).toBeVisible();
  ...
  test('GPS tracking tab renders map simulation panel', async ({ adminPage }) => {
    await adminPage.goto('/transport');
    ...
    await expect(adminPage.locator('text=Live GPS Tracker & Simulator')).toBeVisible({ timeout: 10000 });
    ...
    await expect(adminPage.locator('text=Red Dot = School Campus')).toBeVisible();
  });
