...
test.describe('Authentication Flows', () => {
  ...
  test('should login as admin using adminPage fixture', async ({ adminPage }) => {
  ...
  test('should login as parent using parentPage fixture', async ({ parentPage }) => {
    // Already logged in via fixture
  ...
  test('should lockout account after 5 failed attempts', async ({ page }) => {
    ...
    await expect(errorMsg).toContainText('locked due to too many failed attempts');
  });
