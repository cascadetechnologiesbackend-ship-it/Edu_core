...
test.describe("Student Attendance Flow", () => {
  test("should load attendance page and mark student attendance", async ({
    teacherPage,
  }) => {
    ...
    const markAllPresentBtn = teacherPage.locator(
      'button:has-text("Mark All Present")',
    );
    await expect(markAllPresentBtn).toBeVisible();
    await markAllPresentBtn.click();

    // 6. Enter a remark for the first student
    const firstRemarkInput = teacherPage
      .locator("tbody tr")
      .first()
      .locator('input[placeholder="Optional remarks"]');
    ...
    const successBanner = teacherPage.getByText(
  ...
  test("should display all sections for admin and only assigned sections for teacher", async ({
    adminPage,
    teacherPage,
  }) => {
    // Admin checks
