...
test.describe("Security & Authorization", () => {
  test("should prevent unauthenticated access to protected API routes", async ({
    request,
  }) => {
    // Attempt to access tRPC endpoint directly without session
    const response = await request.post(
      "/api/trpc/students.getStudentProfile",
      {
        data: {
  ...
  test("should enforce rate limits on unauthenticated routes", async ({
    request,
  }) => {
    ...
    for (let i = 0; i < 110; i++) {
      const response = await request.get(
        "/api/trpc/admissions.getUploadUrl?batch=1&input=%7B%220%22%3A%7B%22json%22%3A%7B%22fileName%22%3A%22test.pdf%22%2C%22mimeType%22%3A%22application%2Fpdf%22%7D%7D%7D",
      );
      if (
        response.status() === 429 ||
        (await response.json())[0]?.error?.data?.code === "TOO_MANY_REQUESTS"
      ) {
        hitLimit = true;
        break;
      }
  ...
  test("should prevent IDOR: parent cannot access other child profile", async ({
    parentPage,
    request,
  }) => {
