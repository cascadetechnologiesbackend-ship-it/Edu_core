...
test.describe('Library Management Module Flow', () => {
  ...
  test('admin can view library catalog, members, and transactions', async ({ adminPage }) => {
    ...
    await expect(adminPage.locator('text=Total Catalog Copies')).toBeVisible({ timeout: 10000 });
    ...
    await expect(adminPage.locator('text=Introduction to Algorithms')).toBeVisible();
  ...
  test('admin can add a new book to the catalog', async ({ adminPage }) => {
    ...
    // Fill form
    const modal = adminPage.locator('form:has(h3:has-text("Register New Book Volume"))');
    await expect(modal).toBeVisible();

    await modal.locator('input[placeholder*="Intro to Algorithms"]').fill('Clean Code');
    await modal.locator('input[placeholder*="Thomas H. Cormen"]').fill('Robert C. Martin');
  ...
  test('admin can issue and return book copies', async ({ adminPage }) => {
    ...
    // Issue book
    const checkoutForm = adminPage.locator('h3:has-text("Book Checkout")').locator('..');
    ...
    await expect(adminPage.locator('text=BC-THE-01').first()).toBeVisible({ timeout: 10000 });

    // Return book
    await adminPage.click('button:has-text("Overview")');
    const returnForm = adminPage.locator('h3:has-text("Book Return")').locator('..');
    ...
    await adminPage.click('button:has-text("Transactions")');
    const statusLabel = adminPage.locator('tr:has-text("BC-THE-01")').locator('text=RETURNED');
