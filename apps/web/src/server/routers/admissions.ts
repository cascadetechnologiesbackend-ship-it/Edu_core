...
export const admissionsRouter = createTRPCRouter({
  ...
  getUploadUrl: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const url = await generateUploadUrl(key, process.env.S3_BUCKET || "schoolmitra-docs", input.mimeType);
  ...
  submitApplication: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [application] = await ctx.db.insert(admissionApplications).values({
        ...
        guardianNameEncrypted: input.guardianName ? encryptData(input.guardianName) : null,
        primaryContactMobileEncrypted: encryptData(input.primaryContactMobile),
