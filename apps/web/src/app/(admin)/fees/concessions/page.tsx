  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500">Manage student fee concessions & scholarships.</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow border border-gray-200 dark:border-slate-800 space-y-6 max-w-2xl">
        ...
        <form action={createFeeConcession as any} className="space-y-4">
          ...
          <input type="hidden" name="approvedById" value={activeSchool.id /* using school id as mock approver id for now */} />

          <div>
            <label className="block text-sm font-medium mb-1">Student</label>
            <select name="studentId" required className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select name="concessionType" className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2">
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Concession Name</label>
              <input type="text" name="concessionName" required className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2" placeholder="e.g. RTE 100%" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <select name="appliesTo" className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2">
            ...
            <div>
              ...
              <input type="number" step="0.01" name="discountPercentage" className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Fixed Discount (₹)</label>
              <input type="number" step="0.01" name="discountAmount" className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2" />
            </div>
          </div>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
              {existingConcessions.map((c: any) => (
                <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                  <td className="py-3 font-mono text-xs">{c.studentId.slice(0, 8)}...</td>
                  <td className="py-3">
                  ...
                  <td className="py-3">{c.appliesTo === "ALL" ? "All Fees" : "Specific Head"}</td>
                  <td className="py-3 text-right font-medium text-green-600">
                    {c.discountPercentage ? `${c.discountPercentage}%` : `₹${c.discountAmount}`}
                  </td>
                </tr>
              ))}
              {existingConcessions.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-gray-500">No concessions granted.</td>
                </tr>
              )}
            </tbody>
          </table>
