...
export function CollectForm({ invoices, students, defaultStudentId }: { invoices: any[], students: any[], defaultStudentId?: string }) {
  ...
  const studentInvoices = invoices.filter(i => i.studentId === selectedStudent);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow border border-gray-200 dark:border-slate-800 space-y-6">
        <div>
          <label className="block text-sm font-medium mb-1">Search Student (with pending dues)</label>
          <select 
        ...
        {selectedStudent && (
          <div>
            <h3 className="font-semibold text-gray-700 dark:text-gray-300 mb-3">Pending Invoices</h3>
            {studentInvoices.length === 0 ? (
              <div className="text-sm text-gray-500 bg-gray-50 dark:bg-slate-800 p-4 rounded-md border border-gray-100 dark:border-slate-700">
                ...
                <p className="mt-1 text-xs">If you expected an invoice, please generate missing invoices from the student's Fee Ledger.</p>
              </div>
            ) : (
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-sm bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded">{inv.status}</span>
                        </div>
                        ...
                        <div className="flex justify-between mt-2 text-sm">
                          ...
                          <span className="font-bold text-red-600 dark:text-red-400">Balance: ₹{inv.balanceAmount}</span>
                        </div>
                      </div>
                    </div>
                  </label>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select name="paymentMethod" required className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2">
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Transaction Ref / Cheque No (Optional)</label>
              <input type="text" name="transactionReference" className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2" />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">Remarks (Optional)</label>
              <textarea name="remarks" rows={2} className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2"></textarea>
            </div>

            <button type="submit" disabled={loading} className="w-full bg-green-600 text-white font-medium py-3 rounded-md hover:bg-green-700 disabled:opacity-50 mt-4">
              {loading ? "Processing..." : `Record Payment of ₹${currentInvoice.balanceAmount}`}
            </button>
          </form>
        )}
      </div>
