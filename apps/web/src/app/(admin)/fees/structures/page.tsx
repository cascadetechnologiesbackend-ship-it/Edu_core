  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500">Configure fee heads and structures for {activeYear.label}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow border border-gray-200 dark:border-slate-800 space-y-6">
          ...
          <form action={createFeeHead as any} className="space-y-4">
            ...
            <div>
              ...
              <input type="text" name="name" required className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2" placeholder="e.g. Tuition Fee" />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">Head Type</label>
              <select name="headType" className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2">
            ...
            <button type="submit" className="w-full bg-blue-600 text-white font-medium py-2 rounded-md hover:bg-blue-700">
              Create Fee Head
            </button>
          </form>
          ...
          <div className="mt-6 border-t pt-4">
            ...
            <ul className="space-y-2">
              {existingHeads.map(head => (
                <li key={head.id} className="text-sm flex justify-between bg-gray-50 dark:bg-slate-800 p-2 rounded">
                  <span>{head.name}</span>
                  <span className="text-gray-500 text-xs bg-gray-200 dark:bg-slate-700 px-2 py-0.5 rounded">{head.headType}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        {/* Fee Structures */}
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow border border-gray-200 dark:border-slate-800 space-y-6">
          <h2 className="text-lg font-semibold">2. Assign Structure to Class</h2>
          <form action={createFeeStructure as any} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select name="classId" className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2">
                  {allClasses.map(c => <option key={c.id} value={c.id}>{c.displayName}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Fee Head</label>
                <select name="feeHeadId" className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2">
                  {existingHeads.map(h => <option key={h.id} value={h.id}>{h.name}</option>)}
                </select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select name="term" className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2">
              ...
              <div>
                ...
                <input type="number" step="0.01" name="amount" required className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2" />
              </div>
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">Due Date</label>
              <input type="datetime-local" name="dueDate" required className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2" />
            </div>

            <div className="border p-4 rounded-md space-y-3">
              <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">Late Fee Rules (Optional)</h4>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs mb-1">Type</label>
                  <select name="lateFeeType" className="w-full rounded border border-gray-300 dark:border-slate-600 bg-transparent px-2 py-1 text-sm">
                ...
                <div>
                  ...
                  <input type="number" step="0.01" name="lateFeeAmount" className="w-full rounded border border-gray-300 dark:border-slate-600 bg-transparent px-2 py-1 text-sm" />
                </div>
                <div>
                  <label className="block text-xs mb-1">After (Days)</label>
                  <input type="number" name="lateFeeStartAfterDays" className="w-full rounded border border-gray-300 dark:border-slate-600 bg-transparent px-2 py-1 text-sm" />
                </div>
              </div>
            </div>

            <button type="submit" className="w-full bg-green-600 text-white font-medium py-2 rounded-md hover:bg-green-700">
              Save Structure
            </button>
          </form>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
              ...
              {existingStructures.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-gray-500">No fee structures configured.</td>
                </tr>
              )}
            </tbody>
          </table>
