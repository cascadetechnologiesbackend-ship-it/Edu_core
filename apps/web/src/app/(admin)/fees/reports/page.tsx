...
export default async function FeeReportsPage() {
  ...
  const collectionsByDate = allPayments.reduce((acc, p) => {
    const dateStr = new Date(p.paymentDate).toISOString().split('T')[0] || "Unknown Date";
  ...
  const pendingInvoices = allInvoices.filter(i => ["PENDING", "PARTIAL", "OVERDUE"].includes(i.status) && parseFloat(i.balanceAmount) > 0);
  ...
  const allStudentIds = Array.from(new Set(pendingInvoices.map(i => i.studentId)));
  let mappedStudents: Record<string, any> = {};
  if (allStudentIds.length > 0) {
    const s = await db.query.students.findMany({ where: inArray(students.id, allStudentIds) });
    mappedStudents = s.reduce((acc, student) => {
      acc[student.id] = {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">Monitor collections, outstanding dues, and defaulters.</p>
        </div>
        <ExportButton data={defaulterListForExport} filename="Defaulter_List.xlsx" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow border border-gray-200 dark:border-slate-800">
          <h2 className="text-lg font-semibold mb-4">Recent Daily Collections</h2>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                {dailyCollectionData.map(([date, amount]) => (
                  <tr key={date} className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                    <td className="py-3">{date}</td>
                    <td className="py-3 text-right font-medium text-green-600">{amount.toFixed(2)}</td>
                  </tr>
                ))}
                {dailyCollectionData.length === 0 && (
                  <tr>
                    <td colSpan={2} className="py-8 text-center text-gray-500">No collections recorded yet.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        {/* Ageing Buckets */}
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow border border-gray-200 dark:border-slate-800">
          <h2 className="text-lg font-semibold mb-4">Defaulter Ageing Buckets</h2>
          <div className="space-y-4">
            <div className="flex justify-between items-center p-3 border rounded-lg">
              <span className="font-medium text-gray-700 dark:text-gray-300">0 - 30 Days Overdue</span>
              <span className="text-lg font-bold text-red-500">₹{ageingBuckets["0_30"].toFixed(2)}</span>
            </div>
            <div className="flex justify-between items-center p-3 border rounded-lg">
              <span className="font-medium text-gray-700 dark:text-gray-300">31 - 60 Days Overdue</span>
              <span className="text-lg font-bold text-red-600">₹{ageingBuckets["31_60"].toFixed(2)}</span>
            </div>
            <div className="flex justify-between items-center p-3 border rounded-lg">
              <span className="font-medium text-gray-700 dark:text-gray-300">61 - 90 Days Overdue</span>
              <span className="text-lg font-bold text-red-700">₹{ageingBuckets["61_90"].toFixed(2)}</span>
            </div>
            <div className="flex justify-between items-center p-3 border rounded-lg">
              <span className="font-medium text-gray-700 dark:text-gray-300">90+ Days Overdue</span>
              <span className="text-lg font-bold text-red-800 dark:text-red-900">₹{ageingBuckets["90_plus"].toFixed(2)}</span>
            </div>
          </div>
        </div>
      </div>
