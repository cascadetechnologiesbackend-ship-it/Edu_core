...
export default async function StudentFeeLedgerPage({ params }: { params: { id: string } }) {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <div className="flex items-center gap-3">
            <Link href={`/students/${student.id}`} className="text-blue-600 hover:underline text-sm font-medium">&larr; Back to Profile</Link>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-slate-800">
          ...
          <p className="text-2xl font-bold mt-1 text-green-600">₹{totalConcessions.toFixed(2)}</p>
        </div>
        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-slate-800">
          <p className="text-sm text-gray-500 font-medium">Total Paid</p>
          <p className="text-2xl font-bold mt-1 text-blue-600">₹{totalPaid.toFixed(2)}</p>
        </div>
        <div className="bg-red-50 dark:bg-red-900/10 p-4 rounded-xl shadow-sm border border-red-200 dark:border-red-900/30">
          <p className="text-sm text-red-600 dark:text-red-400 font-medium">Outstanding Balance</p>
          <p className="text-2xl font-bold mt-1 text-red-700 dark:text-red-500">₹{totalBalance.toFixed(2)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-200 dark:border-slate-800 overflow-hidden">
          <div className="p-4 border-b border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-800/50">
            <h2 className="font-semibold text-gray-900 dark:text-white">Fee Invoices</h2>
          </div>
          <div className="divide-y divide-gray-100 dark:divide-slate-800 max-h-[600px] overflow-y-auto">
            {studentInvoices.length === 0 ? (
              ...
              studentInvoices.map(inv => (
                <div key={inv.id} className="p-4 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
          ...
          <div className="divide-y divide-gray-100 dark:divide-slate-800 max-h-[600px] overflow-y-auto">
            {studentPayments.length === 0 ? (
              ...
              studentPayments.map(p => (
                <div key={p.id} className="p-4 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                  <div className="flex justify-between items-start mb-1">
                    <span className="font-medium text-sm">{p.receiptNumber}</span>
                    <span className="font-semibold text-green-600">₹{p.amountPaid}</span>
                  </div>
