...
export default function TimetableTab({
  ...
  const handleCellClick = (day: string, periodNum: number) => {
    ...
    const existing = timetable.find(
      (t) => t.dayOfWeek === day && t.periodNumber === periodNum,
    );
    const times = PERIOD_TIMES.find((pt) => pt.num === periodNum) || {
      start: "08:00",
      end: "08:45",
    };
  ...
  const getCellContent = (day: string, periodNum: number) => {
    ...
    const teacherEmail =
      teachers.find((t) => t.id === record.teacherId)?.email || "No teacher";
    return (
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500">
            Manage or view periods across classes & sections.
          </p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm table-fixed border-collapse">
              <thead className="bg-gray-50 dark:bg-slate-800/50 text-gray-600 dark:text-gray-300 text-xs font-semibold uppercase">
                <tr>
                  <th className="px-4 py-3.5 w-24 border border-gray-100 dark:border-slate-800">
                    Day
                  </th>
                  {PERIOD_NUMBERS.map((p) => {
                    ...
                    return (
                      <th
                        key={p}
                        className="px-4 py-3.5 border border-gray-100 dark:border-slate-800"
                      >
                        Period {p}
              ...
              <>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setCellData({ ...cellData, subjectId: e.target.value })
                    }
                    className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setCellData({ ...cellData, teacherId: e.target.value })
                    }
                    className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCellData({ ...cellData, roomNumber: e.target.value })
                    }
                    className="w-full rounded-md border border-gray-300 dark:border-slate-600 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
                  />
                </div>
