...
export const bookCopies = pgTable(
  ...
  (t) => ({
    barcodeUnique: unique("book_copies_barcode_unique").on(
      t.schoolId,
      t.barcodeNumber,
    ),
  }),
...
export const bookIssues = pgTable(
  "book_issues",
  {
    ...
    libraryMemberId: uuid("library_member_id")
      .notNull()
...
export const bookFines = pgTable("book_fines", {
  ...
  schoolId: uuid("school_id")
    .notNull()
    .references(() => schools.id, { onDelete: "restrict" }),
  bookIssueId: uuid("book_issue_id")
    .notNull()
    .references(() => bookIssues.id, { onDelete: "restrict" }),
