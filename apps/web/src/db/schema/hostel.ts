...
import {
  pgTable, uuid, text, boolean, timestamp, integer, pgEnum, index, unique,
} from "drizzle-orm/pg-core";
import { schools, users } from "./core";

export const roomTypeEnum = pgEnum("room_type", [
  "AC_SINGLE", "AC_DOUBLE", "AC_DORMITORY",
  "NON_AC_SINGLE", "NON_AC_DOUBLE", "NON_AC_DORMITORY",
]);
...
export const hostelRooms = pgTable("hostel_rooms", {
  ...
  schoolRoomUnique: unique("hostel_rooms_unique").on(t.schoolId, t.block, t.roomNumber),
...
export const hostelAllocations = pgTable("hostel_allocations", {
  ...
  roomBedUnique: unique("hostel_allocations_room_bed_unique").on(t.roomId, t.bedNumber),
...
export const visitorLogs = pgTable("visitor_logs", {
  ...
  loggedByUserId: uuid("logged_by_user_id").notNull().references(() => users.id, { onDelete: "restrict" }),
  ...
  schoolDateIdx: index("visitor_logs_school_date_idx").on(t.schoolId, t.checkInTime),
